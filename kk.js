...
sessionStorage.setItem("currentExam",1766251919266)
class StudentExams{
    ...
    constructor(student) {
        ...
        this.finish=true;
        this.examId=sessionStorage.getItem("currentExam");
...
function storeExam(std,key){
    if(!ls.findExam(std.id,sessionStorage.getItem("currentExam"),key)){
...
let studentExam=ls.findExam(student.id,sessionStorage.getItem("currentExam"),"student_exam");
...
//     };
    ...
    // console.log(newQues);
        ...
        let maxScore = 100;
        let answered = false;
        const examDuration = studentExam *60;
        // const questionTimeLimit = examDuration/questions.length; // 
        const questionTimeLimit = 100; // 
